...
interface Props {
  ...
  onAdd: (countryId: string, type: RecordType, content: string, caption?: string, photos?: string[]) => void;
  onClose: () => void;
}
...
export function CountryModal({ countryName, countryId, records, onAdd, onClose }: Props) {
  ...
  function handleAddText() {
    if (!text.trim()) return;
    onAdd(countryId, 'text', text.trim());
    setText('');
    setMode('list');
  }
  ...
  function handleAddImage() {
    ...
    onAdd(countryId, 'image', photos[0], caption.trim() || undefined, photos);
    setPhotos([]);
    setCaption('');
    setMode('list');
  }
